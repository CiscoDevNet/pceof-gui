...
define([], function () {
	...
	function ErrorHandlerService($log, $mdDialog, $mdToast) {
		...
		this.displayMDToast = function(err){
			$mdToast.show({
				...
				templateUrl : 'app/modules/pce/views/error-views/toast.tpl.html',
				locals: {
					'errData': err
				}
			});
		...
		this.displayMDDialog = function(err){
			...
			$mdDialog.show({
				parent: parentEl,
				controller  : 'ErrorHandlerCtrl',
				templateUrl : 'app/modules/pce/views/error-views/dialog.tpl.html',
